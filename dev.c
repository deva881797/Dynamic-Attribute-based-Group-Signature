...
void random_prime_bits(mpz_t result, mpz_t n)
{
    ...
    else
    {
        ...
        while (1)
        {
            ...
            if (mpz_cmp(random, lower_limit) > 0 && mpz_probab_prime_p(random, mpz_get_ui(n))) // REF-11  REF-12  //REF-09
            {
...
void tree_making(mpz_t dummy, mpz_t temp1, mpz_t max, long unsigned int A[], Node *T_d, mpz_t *S, long unsigned int z)
{
    ...
    for (i = 0; i < mpz_get_ui(temp1); i++) // REF-09
    {
        initializeNode(&T[i]);
    }
    for (long unsigned int j = mpz_get_ui(dummy); j < mpz_get_ui(temp1); j++) // REF-09
    {
    ...
    for (long unsigned int j = mpz_get_ui(dummy); j < mpz_get_ui(temp1); j++) // REF-09
    {
        if (j - mpz_get_ui(dummy) == A[i]) // REF-09
        {
            i++;
        }
        else
    ...
    while (i > 0)
    {
        if (mpz_cmp_ui(T[i].name, 0) == 0 && mpz_cmp_ui(T[i - 1].name, 0) == 0) // REF-07
        {
            initializeNode(&T[i / 2 - 1]);
            initializeNode(&T_d[i / 2 - 1]);
        }
        else if (mpz_cmp_ui(T[i - 1].name, 0) == 0) // REF-07
        {
        ...
        else if (mpz_cmp_ui(T[i].name, 0) == 0) // REF-07
        {
            mpf_set_z(temp2, T_d[i / 2 - 1].name); // REF-48
...
void verify_tree(long unsigned int A[], mpz_t dummy, mpz_t temp1, mpz_t max, Node *T_d, unsigned long int z)
{
    ...
    for (i = 0; i < mpz_get_ui(dummy); i++)  // REF-09
    {
    ...
    for (i = 0; i < mpz_get_ui(ret_setup.a); i++)      // REF-09
    {
    ...
    if (z != 0)
    {
        ...
        if (mpz_cmp_ui(s, 0) < 0)    // REF-07
        {
            mpz_abs(R[z - 1].sT, s); // REF-56
            if (mpz_cmp_ui(sT1, 0) < 0)    // REF-07
            {
...
void setup(public *retval, mpz_t k)
{
    ...
    do
    {
        ...
        if (element_cmp(temp_g1, identity_g1) == 0)   // REF-27
        {
            mpz_add_ui(gen, gen, 1);     // REF-24
    ...
    do
    {
        ...
        if (element_cmp(temp_g2, identity_g2) == 0)  // REF-27
        {
            mpz_add_ui(gen, gen, 1);     // REF-24
...
void keygen(char *M, public *ret_val, secret *sec_val)
{
    ...
    printf("  Set S = For particular attributes of secrets you can print\n");
...
void join(public *retval, unsigned long int j, user_output *U_val, reg *rval, secret *sec_val)
{
    ...
    mpz_inits(U_val->upki, rval->upki, U_val->uski, NULL);  //REF-02
    mpz_urandomm(U_val->upki, state, retval->p); // REF-33
    mpz_set(rval->upki, U_val->upki);            // REF-13
    mpz_urandomm(U_val->uski, state, retval->p); // REF-33
    ...
    do
    {
        ...
        for (long unsigned int i = 0; i < mpz_get_ui(a); i++) // REF-09
        {
            if (temp1 == A[i])
            {
                z = 1;
    ...
    for (long unsigned int i = 0; i < mpz_get_ui(retval->a); i++)   // REF-09
    {
        element_init_G1(Ti[i], retval->pairing);       //REF-20
        element_mul_mpz(temp_pow, add, sec_val->S[i]); // REF-37
    ...
    if (element_cmp(temp1, temp2) == 0)       // REF-27
...
void buildtree(public *retval, mpz_t *S)
{
    ...
    while (1)
    {
        ...
        if (mpz_cmp(retval->a, temp1) > 0) // REF-11
        {
            i++;
        }
        else
...
void buildtreevalidity(public *retval)
{
    ...
    long unsigned int *A = (long unsigned int *)malloc(sizeof(long unsigned int) * mpz_get_ui(a)); // REF-09
    long unsigned int k = 0;
    printf("Attributes :");
    do
    {
    ...
    insertionSort(A, mpz_get_ui(a));    // REF-09
    for (k = 0; k < mpz_get_ui(a); k++) // REF-09
    {
...
void sign(user_output *U_val)
{
    ...
    element_set(R[U_val->i - 1].r, r);      // REF-28
    element_set(R[U_val->i - 1].z, z);      // REF-28

    element_t *rho3 = (element_t *)malloc(sizeof(element_t) * 2);  // REF-19
    element_t *rho7 = (element_t *)malloc(sizeof(element_t) * 2);  // REF-19
    element_t rho4, rho5, rho6, rho8, rho9, temp1, temp2;          // REF-19
    element_t temp3, temp4, temp5, temp6, temp7, temp8, temp9, vT; // REF-19
    ...
    element_set(U_val->vT, vT);        // REF-28

    element_set(rho3[0], U_val->Xi_1); // REF-28
    element_set(rho3[1], U_val->Xi_2); // REF-28

    element_add(temp1, private.gamma, U_val->xi); // REF-35
    element_invert(temp2, temp1);                 // REF-36
    if (mpz_cmp_ui(R[U_val->i - 1].sT1, 0) > 0)       // REF-07
    {
    ...
    if (mpz_cmp_ui(R[U_val->i - 1].sT2, 0) > 0)  // REF-07
    {
...
void openuser(user_output *U_val, reg *rval, element_t ok_user)
{
    if (private.alpha1 == ok_user)
    {
        ...
        if (mpz_cmp(verified_message, combined_hash) == 0) // REF-11
        {
            ...
            if (element_cmp(rval->Ai, sigma1_verify[1]) == 0)   // REF-27
            {
...
void traceatt(user_output *U_val, reg *rval, element_t tk_att)
{
    if (private.alpha1_dash == tk_att)
    {
        ...
        if (element_cmp(rho5_verify, sigma5_verify[1]) == 0)   // REF-27
        {
